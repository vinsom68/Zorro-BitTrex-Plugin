...
https://bittrex.com/Api/v2.0/pub/market/GetTicks?marketName=BTC-WAVES&tickInterval=thirtyMin&_=1505694318518
"oneMin", "fiveMin", "thirtyMin", "hour" and "day".
https://bittrex.com/Api/v2.0/pub/market/GetLatestTick?marketName=USDT-BTC&tickInterval=thirtyMin&_=1505694318518
https://bittrex.com/Api/v2.0/pub/market/GetTicks?marketName=USDT-BTC&tickInterval=thirtyMin&_=1505694318517
	...
	try
	{
		...
		if (uuid.length() > 8 || g_DemoTrading)
		{
			...
			if (objJson.at("result").get<picojson::object>().count("QuantityRemaining") > 0)
			{
				...
				CommissionPaid = objJson.at("result").get("CommissionPaid").to_str();
			}


			Log("Trade id: " + uuid + " - " + tradeId + " - Price: " + sRate, 0, true);
			Log("Type: " + orderType + " Price: " + sRate + " Amount: " + sAmount, 0, true);
			Log("Quantity: " + Quantity + " QRemaining: " + QuantityRemaining + " CommissionPaid: " + CommissionPaid, 0, true);
